...
Params.copy_raw_data = false;          % if false, Data.d/hyg/daqinfo are not saved in the .mat file
Params.sines_first = true;          % if false, segment pulse, then sine. 
                                    % if true, segment sine, then pulse, then sine

%estimating noise
Params.low_freq_cutoff = 80;         % exclude data below this frequency
...
Params.minIPI = round((Params.Fs/60)/4)*4; % lowest acceptable IPI, in ticks (even, rounded)
Params.thresh = 3;                    % multiple of smoothed threshold over which pulses are counted
...
Params.NW = 30;                       % time-bandwidth product for tapers
Params.K = 60;                        % numver independent tapers to average over, must be < 2*NW
