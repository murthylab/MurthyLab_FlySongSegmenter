function	[sInf, pInf, wInf, bInf, Song] = postProcessSegmentation_permissive(sInf, data, oneSong, mNoiseSamp, dataScalingFactor, lastSampleToProcess, pulseModelName, pulseModels)
...
if numel(pInf.pSec)>0 % why do that again???
   [~,pInf.pLik] = fitPulse2GivenModel(pInf.pSec, pMod);
   pInf.pLik = pInf.pLik.LLR_best;
end
...
%             pInf.hpWi(i)<0.5)) || (pInf.hpWi(i) > 4 && pInf.ampR(i) > 40);
      if (pInf.pLik(i)<-1 && (pInf.ampR(i)<10 && nPul<type)) || ...
            (pInf.ampR(i)<3 && nPul == 0) || (pInf.pLik(i) < -4 && nPul<type) || ...
            (pInf.ampR(i)>15 && pInf.pLik(i)< 0 && nPul<type) || ...
            (pInf.pLik(i)<0 && (pInf.c2SR(i)<2.5 || pInf.hpWi(i)>5 ||...
            pInf.hpWi(i)<0.5)) || (pInf.hpWi(i) > 4 && pInf.ampR(i) > 40)
         pul2rem(i) = 1;
...
%             || pInf.c2SR(i) < 0.25
      if nDis(2)<150 && amp(2)>amp(1) || ...
            ...
            (type==2 && min(amp(2:3))>3*amp(1) && min(nDis(2:3))<300) || ...
            (type==2 && amp(1)<1.75 && min(nDis(2:3))<250) || ...
            (sum(nDis(2:3)<10000)<type && (amp(1)<2.5 || pInf.hpWi(i)<0.5))|| ...
            (sum(pInf.detM(i,:))==1 && amp(1)<.5) || ...
            (amp(1)<3 && pInf.proxSng(i)>10000) || ...
            pInf.hpWi(i)<0.3 ...
            || pInf.c2SR(i) < 0.25
         pul2rem(i) = 1;
         moveOn = 0;
